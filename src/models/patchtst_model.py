...
class _PatchTST(nn.Module):
    def __init__(
        ...
        num_patches = 1 + (context_window - patch_len) // patch_stride
        self.patch_embed = nn.Linear(patch_len * input_dim, d_model)
        self.position = nn.Parameter(torch.randn(1, num_patches, d_model) * 0.02)
        ...
        self.head = nn.Sequential(
            nn.LayerNorm(num_patches * d_model),
            nn.Linear(num_patches * d_model, horizon),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        tokens = self.patch_embed(patches) + self.position
        encoded = self.encoder(tokens)
        flattened = encoded.reshape(encoded.shape[0], -1)
        return self.head(flattened)


class PatchTSTExperimentModel:
    def __init__(self, config: PatchTSTConfig | None = None, device: str | None = None) -> None:
        self.config = config or PatchTSTConfig()
        self.device = torch.device(default_device(device))
        self.model: _PatchTST | None = None
    ...
    def _build_normalizer(
        ...
        raise ValueError(f"Unsupported PatchTST target_transform: {self.config.target_transform}")
    ...
    def tune_on_tsplit(
        ...
        LOGGER.info(
            "PatchTST tuning start rows=%s buildings=%s sampled_buildings=%s device=%s",
            len(train_frame),
        ...
        grid = [
            {},
            {"d_model": 192, "num_layers": 3, "dropout": 0.1, "learning_rate": 1e-4},
            {"d_model": 128, "num_layers": 2, "dropout": 0.2, "learning_rate": 2e-4},
        ]

        best_config = PatchTSTConfig(**base_config)
        best_score = float("inf")

        for overrides in grid:
            candidate_config = PatchTSTConfig(**(base_config | overrides))
            tuning_config = PatchTSTConfig(**(asdict(candidate_config) | tuning_budget))
            candidate_model = PatchTSTExperimentModel(config=tuning_config, device=str(self.device))
            ...
            LOGGER.info(
                "PatchTST tuning params=%s best_val=%.6f",
                overrides or {"default": True},
                score,
            )
            if score < best_score:
                best_score = score
        ...
        LOGGER.info("PatchTST tuning best_params=%s best_val=%.6f", asdict(self.config), best_score)
        return self.config
    ...
    def fit(self, train_frame: pd.DataFrame) -> "PatchTSTExperimentModel":
        ...
        if len(train_ds) == 0:
            raise RuntimeError("No train windows were generated for PatchTST.")
        LOGGER.info(
            "PatchTST fit start device=%s train_windows=%s valid_windows=%s",
            self.device,
            len(train_ds),
            len(valid_ds),
        )
        ...
        for epoch_idx in range(self.config.max_epochs):
            ...
            for x_batch, y_batch, _ in train_loader:
                ...
                if not torch.isfinite(x_batch).all() or not torch.isfinite(y_batch).all():
                    raise RuntimeError("Non-finite PatchTST training batch detected.")
                optimizer.zero_grad(set_to_none=True)
                pred = self.model(x_batch)
                loss = loss_fn(pred, y_batch)
                if not torch.isfinite(pred).all() or not torch.isfinite(loss):
                    raise RuntimeError("Non-finite PatchTST prediction or loss detected during training.")
                loss.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                for x_batch, y_batch, _ in valid_loader:
                    ...
                    y_batch = y_batch.to(self.device)
                    if not torch.isfinite(x_batch).all() or not torch.isfinite(y_batch).all():
                        raise RuntimeError("Non-finite PatchTST validation batch detected.")
                    pred = self.model(x_batch)
                    if not torch.isfinite(pred).all():
                        raise RuntimeError("Non-finite PatchTST validation prediction detected.")
                    val_losses.append(float(loss_fn(pred, y_batch).item()))

            train_loss = float(np.mean(train_losses)) if train_losses else float("inf")
            val_loss = float(np.mean(val_losses)) if val_losses else float("inf")
            LOGGER.info(
                "PatchTST epoch=%s/%s train_loss=%.6f val_loss=%.6f patience_left=%s",
                epoch_idx + 1,
            ...
            if val_loss < best_val:
                ...
                patience_left -= 1
                if patience_left <= 0:
                    LOGGER.info("PatchTST early stopping triggered at epoch=%s", epoch_idx + 1)
                    break

        if best_state is not None:
            self.model.load_state_dict(best_state)
        self.best_val_loss = best_val
        LOGGER.info("PatchTST fit finished best_val=%.6f", best_val)
        return self
    ...
    def predict_frame(
        ...
        with torch.no_grad():
            ...
            for x_batch, y_batch, _ in loader:
                ...
                if not np.isfinite(pred).all() or not np.isfinite(true).all():
                    raise RuntimeError("Non-finite PatchTST prediction batch detected.")
        ...
        return predictions_to_frame(dataset, pred_denorm, true_denorm, split_name=split_name, model_name="patchtst", fold_id=fold_id)
    ...
    @classmethod
    def load(cls, path: str | Path, device: str | None = None) -> "PatchTSTExperimentModel":
        path = Path(path)
        config_payload = json.loads(path.with_suffix(".config.json").read_text(encoding="utf-8"))
        config = PatchTSTConfig(**(asdict(PatchTSTConfig()) | config_payload))
        ...
        if {"building_min", "building_max"}.issubset(normalizer_payload):
            ...
            raise ValueError("Unsupported PatchTST normalizer payload.")
